...
#define EEPROM_LOGS_START EEPROM.length() - 100 * sizeof(evento) - 1
...
bool pedir_password() {
  ...
  while (true) {
    ...
    pantalla.setCursor(0, 0);
    bool input;
    if (tipoEntrada == KB_INPUT) {
      pantalla.println("password:");
      input = keyboard_input(buffer, 1);
    }
...
bool agregarlogs(char* descripcion) {
  ...
  strncpy(nuevo.desc, descripcion, 15);
  nuevo.id = ++cantidad;
  Serial2.println(nuevo.id);
  EEPROM.put(siguiente_direccion, nuevo);
  EEPROM.put(EEPROM_LOGS_START, cantidad);
  Serial2.println("---Nuevo Logs---");
  Serial2.println(nuevo.desc);
  Serial2.println(nuevo.id);
  Serial2.println("-------------------");
  free(descripcion);
  return true;
